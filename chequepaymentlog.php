...
<head>
<title>Cheque Payment Log</title>
<meta charset="utf-8">
		...
		<div class="header">
			<div id='cssmenu' >
				<ul>
				   ...
				   <li class="active"><a href='paymentlog.php'><span>Cheque Payment Log</span></a></li>
				</ul>
			</div>
			</div>
			</div>
			...
			<div class="wrap-container clearfix">
				...
				<div class='printignore'>
					<div class="crumbs">
						<ul>
							<?php menu($userlevel); ?>
							<li><a href="paymentlog.php">Cheque Payment Log</a></li>
						</ul>
					</div>
					</div>
					<div class="zerogrid">
						...
						<div class='printignore'>
							<h1 class="t-center" style="margin: 15px 0;color: #212121;letter-spacing:2px;font-weight:500;">Cheque Payment Log</h1>
						</div>							
						</div>
					</div>
...
if (!empty($_SESSION["searchtable"]) && !empty($_SESSION["chequepaymentlogacademicyear"]) && !empty($_SESSION["chequepaymentlogsemester"])) {

$searchtable = $_SESSION["searchtable"];
$academicyear = $_SESSION["chequepaymentlogacademicyear"];
$semester = $_SESSION["chequepaymentlogsemester"];
$display = "History";
}
else {
	...
	echo " <h3> <span class='printonly'> $display Cheque Payment Log </span>
	<span class='printignore'>$display <br/> </span> Academic Year $academicyear <br/> $semester  </h3>
	<div class='printignore'>
	<form method='get' action='chequepaymentlog.php'>
	...
	echo "<div class='printignore'>
			<br/>
			<form method='post' action='chequepaymentlog.php'>
			<div class='left'>
			<table>
			<tr>
			...
			<table>
			<tr>
			<td class='label'>Select Status: </td> <td><select name='searchstatus'>
			<option value='All'>All</option>
			...
			<br/>
			<input type='submit' name='searchbyperiod' value='Search by Status'>
			<br/>
			</div>
			<div class='center'>
			<table>
	...
	$transactionnumber = $firstname = $lastname = $paymentdate = $cashierid = $cashierfirstname = $cashierlastname = $accountname = $accountnumber  = $paymentstatus = "";
	$amountdue = $chequeamount = $paymentchange = 0;
	...
	if ($searchtypevalue == "All") {
		...
		$searchtypevalue Cheque Payments <br>
		$displaysearchdatevalue
		...
		$searchstatusvalue = $sqlextension = "";
		
		$searchstatusvalue = $_POST["searchstatus"];
		
		if ($searchstatusvalue != "All") { $sqlextension = "AND paymentstatus = '$searchstatusvalue'"; }
		
		echo " <h3><div class='printignore'> Search by Status<br/></div>
		$searchstatusvalue Cheque Payments</h3> <div class='printignore'> <button onclick='window.print()'>Print</button>
		<hr/> <br/> </div>
	...
	if ($result->num_rows > 0) {
		...
		while ($row = $result->fetch_object()) {
			...
			if ($result1->num_rows == 1) {		
				...
				echo "<tr class='curriculum'>
						...
						<td> $accountname </td>
						<td> $accountnumber </td>
						<td> " , formatcurrency($chequeamount) , " </td>
			...
			echo "<tr class='units'>
					<td colspan=10 align='center'> Total Amount Paid:  " , formatcurrency($totalamountpaid) , " </td> 
					</tr>
				</table><br/>";
		...
		echo "<br/>
				<h3>Change Cheque Payments</h3><br/>
				<form method='get' action='chequepaymentlog.php'>
				<table>
				<tr><td>Select Table: </td>
				<td> <select name='searchtable'>
				<option value='currentchequepaymentlog'>Current Cheque Payment Log</option>
				<option value='historychequepaymentlog'>History Cheque Payment Log</option>
				</select>
				</td>
				</tr>
			...
			if ($searchtable == "historychequepaymentlog") {
				$_SESSION["searchtable"] = "historychequepayments";
			}
			else {
				$_SESSION["searchtable"] = "";
			}
			
			$_SESSION["chequepaymentlogacademicyear"] = $selectedacademicyear;
			$_SESSION["chequepaymentlogsemester"] = $selectedsemester;
			
			$result = "Cheque Payment Log Has Been Successfully Set to Academic Year $selectedacademicyear: $selectedsemester!";
			
			echo "<script> 
				var x = messagealert('$result'); 
				if (x == true) {
					window.location = 'chequepaymentlog.php';
				}
			</script>"; 
			$result = "";
		}
			...
			$_SESSION["searchtable"] = "";
			$_SESSION["chequepaymentsacademicyear"] = "";
			$_SESSION["chequepaymentsemester"] = "";
			
			$result = "Cheque Payment Log Has Been Successfully Reset to the Current Academic Year and Semester!";
