		...
		<div class="header">
			<div id='cssmenu' >
				<ul>
				   ...
				   <li class="active"><a href='refundslog.php'><span>Refunds Log</span></a></li>
				</ul>
			</div>
			</div>
			</div>
			...
			<div class="wrap-container clearfix">
				...
				<div class='printignore'>
					<div class="crumbs">
						<ul>
							<?php menu($userlevel); ?>
							<li><a href="refundslog.php">Refunds Log</a></li>
						</ul>
					</div>
					</div>
					<div class="zerogrid">
						...
						<div class='printignore'>
							<h1 class="t-center" style="margin: 15px 0;color: #212121;letter-spacing:2px;font-weight:500;">Refunds Log</h1>
						</div>							
						</div>
					</div>
<center>
...
if (!empty($_SESSION["userid"]) ) { // checks if the $_SESSION contains userid
	...
	echo " <h3> <span class='printonly'> Refunds Log </span> </h3> <br/>
	...
	echo "<div class='printignore'>
			<br/>
			<form method='post' action='refundslog.php'>
			<div class='left'>
			<table>
			<tr>
			...
			<table>
			<tr>
			<td class='label'>Select Refund Type: </td> <td><select name='searchtype'>
			<option value='All'>All</option>
			...
			<br/>
			<input type='submit' name='searchbytype' value='Search by Refund Type'>
			<br/>
			</div>
			<div class='center'>
			<table>
	...
	$transactionnumber = $firstname = $lastname = $transactiondate = $refundtype = $cashierid = $cashierfirstname = $cashierlastname = "";
	$addedrefundedamount = $totaladdedrefundedamount = 0;
	...
	if ($searchdatetypevalue == "All") {
		...
		$sql = "SELECT * FROM refundslog INNER JOIN users ON users.userid = refundslog.studentnumber";
	}
	else if ($searchdatetypevalue == "Daily") {
		$displaysearchdatevalue = date('F d, Y', strtotime($searchdatevalue));
		
		$sql = "SELECT * FROM refundslog INNER JOIN users ON users.userid = refundslog.studentnumber WHERE (DAY(transactiondate) = DAY('$searchdatevalue')) AND (MONTH(transactiondate) = MONTH('$searchdatevalue')) AND (YEAR(transactiondate) = YEAR('$searchdatevalue'))";
	}
	else if ($searchdatetypevalue == "Weekly") {
		$displaysearchdatevalue = date('W, Y', strtotime($searchdatevalue));
		
		$sql = "SELECT * FROM refundslog INNER JOIN users ON users.userid = refundslog.studentnumber WHERE (WEEK(transactiondate, 3) = WEEK('$searchdatevalue', 3)) AND (YEAR(transactiondate) = YEAR('$searchdatevalue'))";
	}
	else if ($searchdatetypevalue == "Monthly") {
		$displaysearchdatevalue = date('F, Y', strtotime($searchdatevalue));
		
		$sql = "SELECT * FROM refundslog INNER JOIN users ON users.userid = refundslog.studentnumber WHERE (MONTH(transactiondate) = MONTH('$searchdatevalue')) AND (YEAR(transactiondate) = YEAR('$searchdatevalue'))";
	}
	else if ($searchdatetypevalue == "Yearly") {
		$displaysearchdatevalue = date('Y', strtotime($searchdatevalue));
		
		$sql = "SELECT * FROM refundslog INNER JOIN users ON users.userid = refundslog.studentnumber WHERE (YEAR(transactiondate) = YEAR('$searchdatevalue'))";
	}
	
		echo " <h3> <div class='printignore'> Search by Date<br/></div>
		$searchdatetypevalue Refunds <br>
		$displaysearchdatevalue
		...
		if ($searchtypevalue != "All") { $sqlextension = "AND refundtype = '$searchtypevalue'"; }
		
		echo " <h3><div class='printignore'> Search by Refund Type<br/></div>
		$searchtypevalue Refunds</h3> <div class='printignore'> <button onclick='window.print()'>Print</button>
		<hr/> <br/> </div>
		
		<div class='pagebreak'></div>";
		
		$sql = "SELECT * FROM refundslog INNER JOIN users ON users.userid = refundslog.studentnumber $sqlextension";
	...
	if ($result->num_rows > 0) {
		...
		while ($row = $result->fetch_object()) {
			...
			$addedrefundedamount = $row->addedrefundedvalue;
			$totaladdedrefundedamount += $addedrefundedamount;
			...
			if ($result1->num_rows == 1) {		
				...
				echo "<tr class='curriculum'>
						...
						<td> $refundtype </td>
						<td> " , formatcurrency($addedrefundedamount) , " </td>
						<td> $transactiondate </td>
					</tr>";
		}
		if ($searchtypevalue == "All") {
			echo "<tr class='units'>
					<td colspan=8 align='center'> Total Refunds Added/Refunded: " , formatcurrency($totaladdedrefundedamount) , " </td> 
					</tr>
				</table><br/>";
		}
		else if ($searchtypevalue == "Added") {
			echo "<tr class='units'>
					<td colspan=8 align='center'> Total Refunds Added: " , formatcurrency($totaladdedrefundedamount) , " </td> 
					</tr>
				</table><br/>";
		}
			else if ($searchtypevalue == "Refunded") {
			echo "<tr class='units'>
					<td colspan=8 align='center'> Total Refunds Refunded: " , formatcurrency($totaladdedrefundedamount) , " </td> 
					</tr>
				</table><br/>";
		}
